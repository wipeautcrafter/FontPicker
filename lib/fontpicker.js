...
const selectNext = (offset) => {
  ...
  const siblings = children.filter((s) => {
    return (
      !s.classList.contains('fp__hide-s') &&
      !s.classList.contains('fp__hide-c') &&
      !s.classList.contains('fp__hide-l')
    )
  })
...
const onKeyPress = (e) => {
  ...
  if (e.key === 'Enter') {
    ...
    const filtered = fonts.filter(
      (f) =>
        !f.classList.contains('fp__hide-s') &&
        !f.classList.contains('fp__hide-c') &&
        !f.classList.contains('fp__hide-l') &&
        f.dataset.family.toLowerCase().startsWith(e.key)
    )
 ...
 * @param {HTMLElement} _options.parentElement parent element to attach font picker to
 ...
 * @param {Object?} _options.systemFonts system fonts object
 * @param {string} _options.default default font
 */
