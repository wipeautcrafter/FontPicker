//#region long variables
const fontCategories = ["serif", "sans-serif", "display", "handwriting", "monospace"];
const fontLanguages = {
    "arabic": "Arabic",
    "bengali": "Bengali",
...
/**
    ...
    */
FontPicker.initialize = function initialize (_options) {
    ...
    const sortFonts = () => {
        // make favorites and recents the first font items
        [ ...data.fav, ...data.rec ].forEach((font) => {
            const favFont = document.querySelector(`#fp__fonts > [data-family="${font}"]`);
            if (favFont) favFont.parentElement.prepend(favFont);
        });
    };

    // on heart button click
    const onHeartClick = function (e) {
        const font = this.parentElement.dataset.family;
        const isFav = this.classList.toggle("fp__fav");

        if (isFav) {
            data.fav.push(font);
        } else {
            data.fav = data.fav.filter((f) => f !== font);
        }

        storeData();
        sortFonts();

        isFav && this.parentElement.scrollIntoView({ block: "center" });
    };
    ...
    const loadFont = (name) => {
        if (loadedFonts.includes(name)) return;
    ...
    // make a font element selected in the modal
    const selectFont = (font) => {
        if(selectedFont) {
            if(selectedFont === font) return;
            selectedFont.classList.remove("fp__selected");

            // remove heart if not favourite
        ...
        // add heart if not favourite
        if(!font.querySelector(".fp__heart")) {
            const heartButton = createHeartButton();
            font.appendChild(heartButton);
        }

        document.getElementById("fp__preview").style.fontFamily = font.dataset.family;
    };
    ...
    const openPicker = (input) => {
        ...
        // select the element that matches the input value
        selectFont(document.querySelector(`#fp__fonts > [data-family="${input.value}"]`));

        modal.show();
    };

    // set the value of the input element
    const setPicker = (input, value) => {
        loadFont(value);
        input.value = value;
    ...
    const onFontDblClick = function(e) {
        setPicker(openedPicker, this.dataset.family);
        modal.hide();
    };

    // create the modal to use for all font pickers
    const createModal = () => {
        ...
        fpPick.addEventListener("click", () => {
            setPicker(openedPicker, selectedFont.dataset.family);
            modal.hide();
        });
    ...
    FontPicker.create = function create (element) {
        if (!element) {
        ...
        element.open = () => openPicker(element);
        element.set = (value) => setPicker(element, value);
        element.addEventListener("click", element.open);

        setPicker(element, element.value || options.defaultFont);
...
/**
    * @function create
    * @memberof FontPicker
    * @description creates a font picker from an input element or creates a new one
    * @param {HTMLInputElement?} element input element to create the picker from
    * @returns {HTMLInputElement} the created picker
    */
FontPicker.create = function create (element) {
    throw new TypeError("FontPicker has not been initialized!");
};
